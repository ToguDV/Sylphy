...
- **LLM-composed notification text (was TODO #15) — hybrid approach, later upgraded.** (a) At creation, the LLM drafts a base `notificationMessage` persisted on the entity; (b) at fire time, `ReminderMessageComposer` (a service, not an `@Tool`) composes fresh text from the entity fields; (c) fallback chain in `resolveText`: composer OK → composed text; composer throws or returns blank → persisted `notificationMessage`; persisted blank → generic `format(Reminder)` ("Recordatorio: <name>" + description). Guarantee: the notification is never lost; worst case is the generic format. Cost: one LLM call per real fire, zero when Mistral is down. Tests cover all three fallback branches. **Post-MVP upgrade:** the composer now uses the **same effective system prompt as the chat** (`SystemPromptService.getEffectivePrompt`; falls back to its internal default prompt when blank) and injects the **shared conversation memory** (`JpaChatMemory.get` for the owner's `conversationId`, placed system → memory → user, read-only) so notifications inherit the assistant's personality and context. Temperature 0.8, aligned with `AIService`. Tradeoff accepted deliberately: each notification is an LLM call with the full memory context (summaries + raw window) — more tokens per fire.
...
- **Delivered notifications DO enter the conversation history** (decision reversed from MVP, deliberately): `TelegramNotificationDispatcher` records the **actual delivered text** (composed or fallback) as an `AssistantMessage` via `JpaChatMemory.add` right after a successful send, so the thread stays coherent with what the user received and future notifications can riff on it. The composer itself reads memory read-only (system → memory → user); write-back is owned by the dispatcher, the only place that knows the real delivered text. A write-back failure is logged and never marks the delivery as failed (that would cause duplicate re-sends next tick). Known limitation: with several dispatchers in the future, write-back must move up to the scheduler so a single delivery records once; today there is one dispatcher.
